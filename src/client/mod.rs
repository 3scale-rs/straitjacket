use serde::Serialize;
use reqwest::blocking::{ClientBuilder, Client as BClient, Response as BResponse, Request as BRequest, RequestBuilder as BRequestBuilder};
...
impl Client {
    ...
    pub fn new_host_n_token<'h, H, T, D>(host: H, token: T, timeout: D) -> Result<Self, Box<dyn Error>>
        where H: Into<Option<&'h str>>, T: Into<Option<String>>, D: Into<Option<Duration>> {
    ...
    pub fn set_token<S>(&mut self, token: S) -> &Self where S: Into<Option<String>> {
    ...
    fn host_url_result(&self) -> Result<&Url, Box<dyn Error>> {
        self.host_url()
            .ok_or_else(|| From::from("no url"))
    }

    pub fn request_builder<Q, B>(&self, method: Method, path: &str, query: Option<&Q>, body: Option<&B>) -> Result<RequestBuilder, Box<dyn Error>>
    ...
    pub fn request<Q, B>(&self, method: Method, path: &str, query_string: Option<&Q>, body: Option<&B>) -> Result<Request, Box<dyn Error>>
            where   Q: Serialize + ?Sized,
                    B: Serialize + ?Sized
    {
    ...
    pub fn send_request(&self, request: Request) -> Result<Response, Box<dyn Error>> {
        self.client.execute(request).map_err(|e| Box::new(e) as Box<dyn Error>)
    }

    pub fn send<Q, B>(&self, method: Method, path: &str, query_string: Option<&Q>, body: Option<&B>) -> Result<Response, Box<dyn Error>>
        where   Q: Serialize + ?Sized,
                B: Serialize + ?Sized
    {
...
mod tests {
    ...
    mod env {
        ...
        fn get_host_and_token_from_env() -> (String, String) {
            ...
            assert!(host.is_ok() && token.is_ok(), "You need to set the environment variables PORTA_HOST and ACCESS_TOKEN for integration tests to run.");
    ...
    fn it_generates_a_request_descriptor() {
        ...
        let req = c.request(Method::GET, "/admin/api/services/2555417783508/metrics.json", None::<&str>, None::<&str>);
        assert!(req.is_ok());
    }
    ...
    fn it_gets_a_response() {
        let c = setup_client(10);
        let r = c.send(Method::GET, "/admin/api/services/2555417783508/metrics.json", None::<&u8>, None::<&str>);
        assert!(r.is_ok());
    }
}